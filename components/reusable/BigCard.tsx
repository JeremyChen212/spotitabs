...
const PlaylistCard = ({ playlist, hidden }: any) => {
  ...
  return (
    <>
     <Link href={`/playlist/${playlist.id}`} className={`${hidden && "hidden lg:flex"} flex relative flex-col group cursor-pointer hover:scale-[1.01] h-[18rem] md:h-[13rem] xl:h-[14rem]  hover:bg-[#333232] animated-border-1 transition-all bg-[#282828] p-4 rounded-lg text-center overflow-hidden w-full gap-4`}>
